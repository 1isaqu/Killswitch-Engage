...
def random_past_datetime(days_back=730):
    ...
    hour = random.choices(
        range(24),
        weights=[1, 1, 1, 1, 1, 1, 5, 5, 5, 5, 5, 5, 5, 5, 5, 8, 8, 10, 12, 14, 15, 15, 14, 10],
    )[0]
...
async def populate_sessions_v2():
    ...
    try:
        ...
        logger.info(
            f"Inserindo {total_sessions} sessões ({total_sessions/n_users:.1f}/usuário médio)..."
        )
        ...
        logger.info(
            f"✅ Ingestão v2 completa: {actual_count} sessões | {actual_users} usuários | média: {actual_count/actual_users:.1f}/usuário"
        )
