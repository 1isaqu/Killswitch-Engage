...
async def populate():
    ...
    try:
        ...
        df["AppID_real"] = (
            df["Header image"].str.extract(r"apps/(\d+)/")[0].apply(pd.to_numeric, errors="coerce")
        )
        # Se falhar no Header image, tentar no Screenshots (col 37)
        mask_nan = df["AppID_real"].isna()
        if mask_nan.any():
            df.loc[mask_nan, "AppID_real"] = (
                df.loc[mask_nan, "Screenshots"]
                .str.extract(r"apps/(\d+)/")[0]
                ...
                for item in cats:
                    if isinstance(item, list):
                        all_categories.update(
                            [c.strip().capitalize()[:200] for c in item if c.strip()]
                        )
        ...
        await conn.executemany(
            "INSERT INTO desenvolvedores (id, nome, pais) VALUES ($1, $2, $3) ON CONFLICT (nome) DO NOTHING",
            dev_data,
        )
        ...
        for i in range(0, len(jc_data), 5000):
            await conn.executemany(
                "INSERT INTO jogos_categorias (jogo_id, categoria_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
                jc_data[i : i + 5000],
            )
        ...
        for i in range(5000):
            ...
            user_data.append(
                (
                    name,
                    f"{name.lower()}@gameverse.io",
                    random.choice(["BR", "US", "DE", "JP"]),
                    random.randint(18, 60),
                    "premium",
                )
            )
        await conn.executemany(
            "INSERT INTO usuarios (nome, email, pais, total_horas, tipo_assinatura) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (email) DO NOTHING",
            user_data,
        )
        ...
        top_games = list(game_uuid_map.values())[
            :2000
        ]  # Limitar a 2k jogos populares para densidade
        biblioteca_data = []
        ...
        for i in range(0, len(biblioteca_data), 5000):
            await conn.executemany(
                "INSERT INTO biblioteca (usuario_id, jogo_id, tipo_aquisicao, preco_pago) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING",
                biblioteca_data[i : i + 5000],
            )
