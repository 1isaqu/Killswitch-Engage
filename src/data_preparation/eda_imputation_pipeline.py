...
def impute_genres_layer_1(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if train_mask.sum() > 100:
        ...
        train_text = (
            df.loc[train_mask, "About the game"].fillna("")
            + " "
            + df.loc[train_mask, "Tags"].fillna("")
        )
        ...
        if test_mask.any():
            test_text = (
                df.loc[test_mask, "About the game"].fillna("")
                + " "
                + df.loc[test_mask, "Tags"].fillna("")
            )
        ...
        if test_mask.any():
            df.loc[test_mask, "Genres_b2"] = pd.Series(
                clf_knn.predict(X_num[test_mask]), index=df[test_mask].index
            ).astype("string")
...
def impute_tags_layer_1(df: pd.DataFrame) -> pd.DataFrame:
    ...
    known_idx = (
        ...
        .sample(min(5000, df["Tags"].notna().sum()), random_state=42)
        .index
    )
...
def apply_ensemble_imputation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in target_cols:
        ...
        for tech in tech_cols:
            ...
            valid_idx = (
                y_pred.replace("", pd.NA).replace("nan", pd.NA).replace("<NA>", pd.NA).notna()
            )
...
def apply_hierarchical_fallback(df: pd.DataFrame) -> pd.DataFrame:
    ...
    targets_defaults = {"Genres": "Indie", "Tags": "singleplayer", "Categories": "Single-player"}
    ...
    for col, default_val in targets_defaults.items():
        ...
        dev_mode = (
            df.dropna(subset=[col, "Developers"]).groupby("Developers")[col].agg(get_mode_value)
        )
