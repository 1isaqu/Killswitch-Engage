...
def q1_price_by_category(df):
    ...
    # Estatísticas
    stats_df = (
        df_cat.groupby("Categories")["Price"]
    ...
    write_report(
        "Conclusão: Existem diferenças SGNIFICATIVAS nos preços entre categorias."
        if p_val < 0.05
        else "Conclusão: NENHUMA diferença significativa de preço entre as categorias."
    )
...
def q2_playtime_by_genre(df):
    ...
    stats_df = (
        df_gen.groupby("Genres")["Average playtime forever"]
...
def q4_achievements_retention(df):
    ...
    pearson_r, p_pearson = stats.pearsonr(
        df_ach["Achievements"], df_ach["Average playtime forever"]
    )
    ...
    df_ach["Achiev_Tier"] = pd.cut(
        df_ach["Achievements"],
        bins=[0, 10, 50, 150, 500],
        labels=["Baixo (1-10)", "Médio (11-50)", "Alto (51-150)", "Extremo (151-500)"],
    )
...
def q5_price_vs_score(df):
    ...
    df_ps["Price_Tier"] = pd.cut(
        df_ps["Price"],
        bins=[-1, 0, 5, 20, 60, 500],
        labels=["Gratuito", "Baixo (<$5)", "Médio ($5-20)", "Alto ($20-60)", "AAA ($60+)"],
    )
    ...
    groups = [
        group["Score"].values for name, group in df_ps.groupby("Price_Tier") if len(group) > 0
    ]
    f_stat, p_val = stats.f_oneway(*groups)
...
def q6_popularity_vs_volume(df):
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(
        df_pop["Log_Owners"], df_pop["Log_Reviews"]
    )
...
def q7_multivariate_partial(df):
    ...
    df_multi = (
        df[["Price", "Metacritic score", "Average playtime forever", "Achievements"]]
        .dropna()
        .copy()
    )
    ...
    df_multi["Is_Indie"] = (
        df.loc[df_multi.index, "Genres"]
        .astype(str)
        .str.contains("Indie", case=False, na=False)
        .astype(float)
    )

    # Controlamos Preço vs Metacritic pelo Fato do Jogo ser Indie ou não. (Partial Covariate must be float)
    pc_price_score = pg.partial_corr(
        data=df_multi, x="Price", y="Metacritic score", covar="Is_Indie"
    )

    # Controlamos Progressão vs Retenção pelo Fator de Qualidade (Score)
    pc_achiev_time = pg.partial_corr(
        data=df_multi, x="Achievements", y="Average playtime forever", covar="Metacritic score"
    )
